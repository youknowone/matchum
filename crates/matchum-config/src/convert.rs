use crate::matchum_config::{
    MatchumConfig, MatchumDictDef, MatchumLanguageSetting, MatchumOverride,
};
use crate::settings::{
...
pub fn to_cspell_settings(config: &MatchumConfig, config_dir: &Path) -> CSpellSettings {
    ...
    // Auto-discover .matchum/dict/*.txt as dictionary definitions
    discover_dict_dir(&config_dir.join(".matchum/dict"), &mut s);
...
/// Convert `CSpellSettings` to `MatchumConfig`, extracting inline words to files.
///
/// Returns the config and a list of word files to create.
pub fn from_cspell_settings(settings: &CSpellSettings) -> (MatchumConfig, Vec<WordFile>) {
    let mut config = MatchumConfig::default();
    ...
    // Convert patterns (StringOrList → Vec<String> not needed in matchum.toml, skip named patterns)
...
fn override_word_path(filename_glob: &str, kind: &str, index: usize) -> String {
    ...
    format!(".matchum/override-{slug}-{kind}.txt")
}
...
mod tests {
    ...
    fn roundtrip_simple() {
        let settings = CSpellSettings {
            ...
            words: vec!["matchum".into(), "cspell".into()],
        ...
        assert_eq!(config.words_file.as_deref(), Some(".matchum/words.txt"));
        assert!(config.flag_words_file.is_none());
        assert_eq!(config.dictionaries, vec!["en_us"]);

        assert_eq!(files.len(), 1);
        assert_eq!(files[0].0, ".matchum/words.txt");
        assert_eq!(files[0].1, "matchum\ncspell\n");
    ...
    fn override_word_file_naming() {
        ...
        assert_eq!(
            ...
            Some(".matchum/override-rs-words.txt")
        );
        assert_eq!(files.len(), 1);
    ...
    fn discover_dict_dir_loads_words_only() {
        ...
        let config = MatchumConfig::default();
    ...
    fn discover_dict_dir_missing_dir_is_noop() {
        let dir = tempfile::tempdir().unwrap();
        // No .matchum/dict directory exists
