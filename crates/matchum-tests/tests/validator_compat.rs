...
mod cspell_compat_mode {
    use super::*;

    fn make_cspell_compat_validator(words: &[&str]) -> Validator {
    ...
    #[test]
    fn accepts_mark_ui_as_ready_via_word_splitter() {
        let v = make_cspell_compat_validator(&["mark", "ui", "as", "ready"]);
        let issues = v.validate_text("markUIAsReady");
        assert!(
            issues.is_empty(),
            "cspell compat mode should accept markUIAsReady: {:?}",
            issue_words(&issues)
        );
    }

    #[test]
    fn accepts_refactor_with_trailing_d() {
        let v = make_cspell_compat_validator(&["refactor"]);
        let issues = v.validate_text("REFACTOR'd");
        assert!(
            issues.is_empty(),
            "cspell compat mode should accept REFACTOR'd: {:?}",
            issue_words(&issues)
        );
    }

    #[test]
    fn accepts_n_log_via_escape_split() {
        let v = make_cspell_compat_validator(&["log"]);
        let issues = v.validate_text("\\n'log'");
        assert!(
            issues.is_empty(),
            "cspell compat mode should accept \\\\n'log': {:?}",
            issue_words(&issues)
        );
    }
}
...
mod e2e_real_dict {
    ...
    #[test]
    fn validate_acronym_boundary_with_cspell_style_splitter() {
        let v = make_validator(&["postgre"], &[], &[]);

        let issues = v.validate_text("DBforPostgreSQL");
        assert!(
            issues.is_empty(),
            "cspell-style acronym boundary search should accept DBforPostgreSQL: {:?}",
            issue_words(&issues)
        );
    }

    #[test]
    fn validate_mark_ui_as_ready_with_cspell_style_splitter() {
        let v = make_validator(&["mark", "ready"], &[], &[]);

        let issues = v.validate_text("markUIAsReady");
        assert!(
            issues.is_empty(),
            "cspell-style camel splitter should accept markUIAsReady: {:?}",
            issue_words(&issues)
        );
    }
...
mod compound_word_modes {
    ...
    #[test]
    fn separate_words_accepts_recursive_same_dict_compound() {
        // cspell's allowCompoundWords supports multi-part compounds inside one dictionary.
        let colors = make_named_dict(&["red", "green", "blue", "yellow"]);
        ...
        let v = Validator::new_named(vec![("colors".to_string(), colors, true)], config);

        let issues = v.validate_text("redgreenblue");
        assert!(
            issues.is_empty(),
            "SeparateWords should accept recursive same-dict compounds: {:?}",
            issue_words(&issues)
        );
    }
