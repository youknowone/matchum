...
mod forbidden_in_dict {
    ...
    #[test]
    fn dict_forbidden_case_sensitive() {
        let mut dict = HashDictionary::new(true);
        dict.add_word("Color");
        dict.add_forbidden("Colour");
        let v = Validator::new(vec![Box::new(dict)], ValidatorConfig::default());
        let issues = v.validate_text("Colour colour");
        assert!(
            issues
                .iter()
                .any(|issue| issue.word == "Colour" && issue.is_forbidden)
        );
        assert!(
            !issues
                .iter()
    ...
    #[test]
    fn dict_no_suggest_case_sensitive() {
        let mut dict = HashDictionary::new(true);
        dict.add_word("Color");
        dict.add_no_suggest("Colour");
        let v = Validator::new(vec![Box::new(dict)], ValidatorConfig::default());
        let issues = v.validate_text("Colour colour");
        assert!(!issues.iter().any(|issue| issue.word == "Colour"));
        assert!(issues.iter().any(|issue| issue.word == "colour"));
    }

    #[test]
    fn dict_no_suggest_overrides_forbidden() {
        let mut dict = HashDictionary::new(false);
        dict.add_forbidden("colour");
        dict.add_no_suggest("colour");
        let v = Validator::new(vec![Box::new(dict)], ValidatorConfig::default());
        ...
        assert!(
            issues.is_empty(),
            "no_suggest should suppress forbidden reporting: {:?}",
            issue_words(&issues)
        );
    }
...
// LocalWords (Emacs-style) is NOT recognized by cspell.
// Skipped for cspell compatibility.
