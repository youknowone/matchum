...
// ============================================================
// cspell wordSplitter.test.ts ports
// ============================================================

mod cspell_word_splitter {
    use super::*;
    use std::collections::HashSet;

    fn sample_words() -> HashSet<&'static str> {
        [
            ".tensor",
            "torch",
            "'twas",
    ...
    fn has(word: splitter::Word<'_>) -> bool {
        let words = sample_words();
        let text = word.text;
        text.len() < 3 || words.contains(text) || words.contains(text.to_lowercase().as_str())
    }

    fn split_text(text: &str) -> Vec<String> {
        let line = splitter::Word { text, offset: 0 };
        splitter::split(line, 0, has, None)
            .words
            .into_iter()
            .map(|w| w.text.to_string())
            .collect()
    ...
    #[test]
    fn find_next_word_text_skips_numeric_literals() {
        let word = splitter::find_next_word_text("1.25e7 hello", 0);
        assert_eq!(word.text, "hello");
        assert_eq!(word.offset, 7);
    }

    #[test]
    fn split_mark_ui_as_ready() {
        assert_eq!(
            split_text("markUIAsReady"),
            vec!["mark", "UI", "As", "Ready"]
        );
    }

    #[test]
    fn split_refactor_d() {
        // Splitter breaks on apostrophe, yielding REFACTOR + d.
        // The single-letter "d" is filtered by minWordLength at validation time,
        // not by the splitter itself.
        assert_eq!(split_text("REFACTOR'd"), vec!["REFACTOR", "d"]);
    }

    #[test]
    fn split_n_log_with_escape_prefix() {
        let line = splitter::Word {
            text: "\\n'log'",
            offset: 0,
        };
        let words: Vec<String> = splitter::split(line, 1, has, None)
            .words
            .into_iter()
            .map(|w| w.text.to_string())
            .collect();
        assert_eq!(words, vec!["log"]);
    }

    #[test]
    fn split_numeric_hyphenated_word() {
        assert_eq!(split_text("32bit-checksum"), vec!["bit", "checksum"]);
    }
}
