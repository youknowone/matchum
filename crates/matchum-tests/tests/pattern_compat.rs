...
mod escape_patterns {
    ...
    fn unicode_escape_not_skipped_by_default() {
        // HexValues (\\uXXXX) is NOT in cspell's default ignoreRegExpList.
        // \uBABC should NOT be skipped — BABC should be flagged.
        let dict = make_dict(&["const", "value"]);
        let v = Validator::new(vec![dict], ValidatorConfig::default());
        ...
        assert!(
            words.contains(&"BABC"),
            "BABC should be flagged (HexValues not in default skip patterns): {:?}",
            words
        );
    }
