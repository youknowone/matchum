...
pub struct CheckOptions {
    ...
    /// `ruspell check` uses `default_settings()`, `ruspell cspell` uses `CSpellSettings::default()`.
...
/// Run check with pre-built settings (used by cargo-ruspell).
...
fn run_collect_issues(
    ...
    let compiled_overrides = ruspell_config::overrides::compile_overrides(settings);
    ...
    let word_caches: std::sync::Mutex<
        std::collections::HashMap<String, ruspell_core::validator::WordCache>,
...
fn load_settings(
    ...
    match resolver::find_config_prioritized_with_stop(&search_dir, stop_config_search_at) {
        resolver::ConfigFound::Ruspell(path) => {
            let config_dir = path.parent().map(|p| p.to_path_buf());
            let settings =
                resolver::load_ruspell_config(&path).context("failed to load ruspell.toml")?;
            Ok((settings, config_dir))
        }
        resolver::ConfigFound::Cspell(path) => {
            eprintln!(
                "hint: using cspell config. Run `ruspell init --from-cspell` to migrate to ruspell.toml."
            );
            let config_dir = path.parent().map(|p| p.to_path_buf());
