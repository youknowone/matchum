...
pub fn run_review(
    ...
    // 1. Load ruspell config for writable dicts
    let (ruspell_config, ruspell_config_path, config_dir) = load_ruspell_config(config_path)?;
    let mut writable_dicts = collect_writable_dicts(&ruspell_config, &config_dir);
...
fn dispatch_command(
    ...
    config_dir: &Path,
    ruspell_config_path: Option<&Path>,
    typos_toml: &Path,
    ...
    match cmd {
        "a" | "accept" => {
            if arg == Some("0") {
                let new_dict = create_new_dict(reader, config_dir, ruspell_config_path)?;
                eprintln!(
...
fn load_ruspell_config(
    config_path: Option<&Path>,
) -> Result<(
    ruspell_config::ruspell_config::RuspellConfig,
    Option<PathBuf>,
    PathBuf,
)> {
    ...
    let path = if let Some(p) = config_path {
        ...
        match resolver::find_config_prioritized(&cwd) {
            resolver::ConfigFound::Ruspell(p) => p,
            _ => {
                return Ok((
                    ruspell_config::ruspell_config::RuspellConfig::default(),
                    None,
                    cwd,
                ));
            }
    ...
    if ext != "toml" {
        // cspell JSON — no RuspellConfig to extract, return defaults
    ...
    let config: ruspell_config::ruspell_config::RuspellConfig =
...
fn collect_writable_dicts(
    config: &ruspell_config::ruspell_config::RuspellConfig,
    config_dir: &Path,
) -> Vec<WritableDict> {
    let mut dicts = Vec::new();
    let mut seen = std::collections::HashSet::new();

    // Auto-discover .ruspell/dict/*.txt
    let dict_dir = config_dir.join(".ruspell/dict");
...
/// Interactively create a new word-list file and register it in ruspell.toml.
fn create_new_dict(
    reader: &mut impl BufRead,
    config_dir: &Path,
    ruspell_config_path: Option<&Path>,
    ...
    // Bare name → .ruspell/dict/<name>.txt, otherwise use as-is
    let rel_path = if name_input.contains('/') || name_input.contains('.') {
        name_input.to_string()
    } else {
        format!(".ruspell/dict/{}.txt", name_input)
    };
    ...
    // Update ruspell.toml with words_file if it exists
    if let Some(config_path) = ruspell_config_path {
...
fn disable_in_files(occurrences: &[Occurrence]) -> Result<usize> {
    ...
    for file_path in files {
        ...
        let already = lines.iter().take(5).any(|l| {
            matches!(
                ruspell_config::directives::parse_directive(l),
                Some(ruspell_config::directives::Directive::Disable)
            )
        });
...
fn ignore_in_files(occurrences: &[Occurrence], word: &str) -> Result<usize> {
    ...
    for file_path in files {
        ...
        for line in lines.iter_mut().take(20) {
            if let Some(ruspell_config::directives::Directive::Ignore(ref words)) =
                ruspell_config::directives::parse_directive(line)
            {
...
mod tests {
    ...
    fn collect_writable_dicts_auto_discover() {
        ...
        let config = ruspell_config::ruspell_config::RuspellConfig::default();
    ...
    fn collect_writable_dicts_words_file() {
        ...
        std::fs::write(dir.path().join(".ruspell/words.txt"), "hello\n").unwrap();
        let config = ruspell_config::ruspell_config::RuspellConfig {
            words_file: Some(".ruspell/words.txt".to_string()),
            ..Default::default()
        };
        let dicts = collect_writable_dicts(&config, dir.path());
    ...
    fn collect_writable_dicts_missing_file_ignored() {
        let dir = tempfile::tempdir().unwrap();
        let config = ruspell_config::ruspell_config::RuspellConfig {
            words_file: Some(".ruspell/nonexistent.txt".to_string()),
            ..Default::default()
        };
        let dicts = collect_writable_dicts(&config, dir.path());
        assert!(dicts.is_empty());
    ...
    fn collect_writable_dicts_dedup() {
        ...
        let config = ruspell_config::ruspell_config::RuspellConfig {
            words_file: Some(".ruspell/dict/words.txt".to_string()),
            ..Default::default()
        };
        let dicts = collect_writable_dicts(&config, dir.path());
        assert_eq!(dicts.len(), 1);
    ...
    fn create_new_dict_bare_name() {
        ...
        assert!(dict.path.ends_with(".ruspell/dict/matchum.txt"));
    ...
    #[test]
    fn create_new_dict_updates_ruspell_toml() {
        let dir = tempfile::tempdir().unwrap();
